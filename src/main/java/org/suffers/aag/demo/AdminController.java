...
public class AdminController {
    ...
    private boolean isRowEditable(Task task) {
        return task != null && !task.getCreator().getRole().equals("OWNER");
    }
    ...
    private void configureMainTable() {
        ...
        assigneeColumn.setCellFactory(createUserComboBoxCell());
        ...
        List<User> userList = jdbcDao.getProgrammersBySuperiorId(currentUser.getId());
        userList.add(currentUser);
        allUsers.setAll(userList);
        tasksTable.setItems(FXCollections.observableArrayList(jdbcDao.getAllTasksByMonthAndAdmin(currentUser.getId())));
    ...
    private Callback<TableColumn<Task, User>, TableCell<Task, User>> createUserComboBoxCell() {
        return column -> new TableCell<Task, User>() {
...

            {
                ...
                comboBox.setButtonCell(new ListCell<User>() {
                    ...
                    protected void updateItem(User user, boolean empty) {
                        super.updateItem(user, empty);
                        setText(user != null ? user.getUsername() : "Выберите...");
                    }
                });

                comboBox.setOnAction(event -> handleComboBoxChange());
            }
            ...
            private void handleComboBoxChange() {
                ...
                if (isEditing()) {
                    ...
                    if (task != null) {
                        handleAssigneeChange(task);
                    }
                }
            }

            private void handleAssigneeChange(Task task) {
            ...
            public void startEdit() {
                if (isRowEditable(getTableRow().getItem())) {
                    ...
                    super.startEdit();
                    comboBox.setStyle("-fx-border-color: #d0e3f0; -fx-background-color: white;");

                    comboBox.setItems(allUsers);

                    if (comboBox.getValue() == null && getItem() != null) {
                        comboBox.setValue(getItem());
                    }
